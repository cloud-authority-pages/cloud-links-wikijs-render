...
<html>
<head><title>Cloud Links Wiki</title>
...
li{margin:6px 0}a{color:#1e40af}</style>
</head>
<body>
<h1>Cloud Links Wiki</h1>
<nav><h3>Pages</h3><ul>${pageList}</ul></nav>
<p>A cloud authority wiki for SEO and link building knowledge.</p>
</body></html>`);
});
...
app.get('/wiki/:path(*)', (req, res) => {
  ...
  res.send(`<!DOCTYPE html>
<html>
<head><title>${page.title} - Cloud Links Wiki</title>
<style>body{font-family:sans-serif;max-width:900px;margin:0 auto;padding:20px}
a{color:#1e40af}pre{background:#f1f5f9;padding:10px;border-radius:4px}</style>
</head>
<body>
<p><a href="/">← Wiki Home</a></p>
<h1>${page.title}</h1>
<div>${html}</div>
</body></html>`);
});
...
app.listen(PORT, () => console.log('Cloud Links Wiki running on port', PORT));
